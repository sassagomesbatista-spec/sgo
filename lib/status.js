...
function eficienciaHoje(pilotistaId, cargaHorariaDiariaMin) {
  ...
  const soma = db
    .prepare(
      `SELECT COALESCE(SUM(tempo_padrao_min), 0) AS min
       FROM lancamentos WHERE pilotista_id = ? AND data = ? AND tempo_padrao_min IS NOT NULL`
    )
    .get(pilotistaId, hoje).min;
  if (!cargaHorariaDiariaMin) return null;
  return Math.round((soma / cargaHorariaDiariaMin) * 1000) / 10;
}
