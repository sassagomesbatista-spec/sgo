...
CREATE TABLE IF NOT EXISTS pilotistas (
  ...
  carga_horaria_diaria_min INTEGER NOT NULL DEFAULT 480
);
...
-- tempo_padrao_* = SAM (Standard Allowed Minutes), em minutos: quanto tempo
-- essa peça deveria levar em cada nível, usado pra calcular eficiência
-- (peças produzidas x SAM / minutos trabalhados x 100 — método padrão da
-- indústria de confecção). Fica NULL até alguém preencher; sem isso, a
-- eficiência dessa peça simplesmente não aparece (não trava nada).
CREATE TABLE IF NOT EXISTS tipos_peca (
  ...
  preco_medio REAL,
  preco_dificil REAL,
  tempo_padrao_simples REAL,
  tempo_padrao_medio REAL,
  tempo_padrao_dificil REAL
);
...
try {
  ...
  // Eficiência (SAM): tempo padrão da peça (minutos) e o % resultante,
  // gravados no lançamento pra ficar no histórico mesmo se o tempo padrão
  // do tipo de peça mudar depois.
  ensureColumn('lancamentos', 'tempo_padrao_min', 'REAL');
  ensureColumn('lancamentos', 'eficiencia_pct', 'REAL');
  ensureColumn('tipos_peca', 'tempo_padrao_simples', 'REAL');
  ensureColumn('tipos_peca', 'tempo_padrao_medio', 'REAL');
  ensureColumn('tipos_peca', 'tempo_padrao_dificil', 'REAL');
  ensureColumn('pilotistas', 'carga_horaria_diaria_min', 'INTEGER DEFAULT 480');
} catch {
  // Same build-time race as above.
}
