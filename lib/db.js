...
try {
  ...
  ensureColumn('usuarios', 'senha_migrada', "INTEGER NOT NULL DEFAULT 0");
} catch {
  // Same build-time race as above.
}
...
// Migração única: a senha "reset1234" usada no reset de 28/08 ficou em
// texto puro no histórico do commit deste repositório, que é PÚBLICO no
// GitHub — ou seja, deixou de ser segredo no instante em que foi commitada.
// Em vez de repetir isso, gera uma senha aleatória nova, salva só o hash
// no banco e imprime o valor uma única vez no log do servidor (nunca no
// código/git) pra ser recuperada e repassada manualmente. O bloco anterior
// que forçava a senha TODA vez que o servidor subia também foi removido —
// era esse o motivo dela "voltar sozinha" depois de trocada pela tela Conta.
// A coluna `senha_migrada` garante que isso rode uma única vez, mesmo que
// o container reinicie várias vezes.
function migrarSenhaDefinitivaAdmin() {
  const admin = db.prepare('SELECT senha_migrada FROM usuarios WHERE usuario = ?').get('admin');
  if (!admin || admin.senha_migrada) return;
  const novaSenha = randomBytes(9).toString('base64url');
  db.prepare('UPDATE usuarios SET senha_hash = ?, senha_migrada = 1 WHERE usuario = ?').run(
    hashPassword(novaSenha),
    'admin'
  );
  console.log(`[SETUP-UNICO] Nova senha definitiva do admin: ${novaSenha}`);
}

try {
  migrarSenhaDefinitivaAdmin();
} catch {
  // Same build-time race as above.
}
